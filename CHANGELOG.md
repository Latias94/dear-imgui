...
See the [compatibility guide](docs/COMPATIBILITY.md), [custom backend guide](docs/CUSTOM_BACKENDS.md), and [runnable examples](examples/README.md) for detailed integration patterns.

### Highlights

- Upgraded the native baseline to Dear ImGui v1.92.9b docking via cimgui and aligned Dear ImGui Test Engine with the 1.92.9 series. This includes upstream fixes for drag/slider text-edit flicker, table border alignment, and legacy `ImDrawData::CmdListsCount` reporting. Generated native and WASM bindings remain reproducible from pinned source revisions.
...
- `dear-app` now offers `run_ui` for small applications and a state-owning `Application` runtime for lifecycle-aware applications. It admits or recovers the main surface before opening a UI frame, and its optional Test Engine integration owns the complete render/present transaction. Docking is explicit through `DockingConfig`. [PR #46](https://github.com/Latias94/dear-imgui-rs/pull/46)
- Numeric widget formats are now validated and typed by their Rust value through `NumericFormat<T>`. Core, ImPlot, and reflect reject format strings that could consume the wrong C variadic arguments, while numeric text input uses checked conversions instead of relying on out-of-range `sscanf` behavior.
...
| `WgpuRenderer::new_frame()` | Remove the call. Main rendering opens resources from `RenderedFrame`; call `reconcile_frame(&mut frame)` before default multi-viewport callbacks. |
...
| Exact upstream Demo, Metrics, or Style windows for font diagnostics | Normal `show_demo_window`, `show_metrics_window`, and style editors remain safe and omit only destructive font-atlas panels; use `show_upstream_*` or `show_font_atlas_debug_panel` explicitly and unsafely when required. [PR #52](https://github.com/Latias94/dear-imgui-rs/pull/52) |
...
| Pinned renderer storage, free callback installers, manual callback clearing, or renderer attach without a platform owner | Use the owning platform and renderer runtime, pass the live `WinitPlatformRuntime` or `Sdl3PlatformBackend` to WGPU/Ash attach, and call the ordered, fallible shutdown path. Custom compatible platforms use the documented unsafe `attach_unchecked` escape hatch. |
| `sdl3_poll_event_ll()` or safe SDL backend processing of `&SDL_Event` | Poll owned `sdl3::event::Event` values through `EventPump` and pass them to `process_event`. Raw callback integrations use `unsafe process_raw_event` and must uphold the SDL union, payload lifetime, thread, and backend provenance contracts. |
| Multiple simultaneously active SDL3 ImGui platform runtimes | Keep one SDL3 platform runtime active per process. A second initialization now returns `Sdl3BackendError::PlatformSessionOccupied`; shut down the first runtime before attaching another Context. |
| `dear-app` `AppBuilder`, `RunnerCallbacks`, `RunnerConfig`, `run_simple`, `run_with_callbacks`, or presentation hooks | Use `AppConfig` with `run_ui` or implement `Application` and call `dear_app::run`. Enable `test-engine` and return the engine from `Application::test_engine` instead of manually bracketing presentation. |
| Bevy backend config/status resources, camera markers, helper setup, public begin/end schedules or viewport queues, manual image IDs, and wrapper extraction | Use `ImguiPluginConfig`, `ImguiUi`, `ImguiContextConfig`, explicit render/input routes, `ImguiTexture` leases, `ImguiNativeViewportSupport::get(context_id)`, and retryable `ImguiContexts::remove`; renderer and viewport queue storage is private. |
| A raw Bevy `ScheduleLabel` passed to `ImguiContextConfig::new` | Create one `ImguiContextPass`, clone it for `App::add_systems`, and move it into `ImguiContextConfig::new`; call `ImguiAppExt::shutdown_imgui` before terminal `App` teardown when deterministic cross-world cleanup is required. |
...
| `PostEditor::{set_node_pos_grid,set_node_pos_screen}`, direct attribute creation on `NodeEditor`, or assuming `editor()` immediately begins native submission | Configure `NodeEditorSetup`, call `begin_nodes()`, and create attributes from the matching `NodeToken`. Use `PostEditor::center_on_submitted_node` for next-frame panning and handle the boolean result from ID-specific post-frame operations. |
...
| Raw strings passed to numeric `display_format` / `format` methods, ImPlot value labels, or reflect numeric settings | Construct `NumericFormat<T>` and pass it to `display_format`, or use the fallible `try_display_format` convenience methods. ImPlot uses ASCII-only `FloatFormat` and length-bounded `AxisFormat`; use `colormap_*_with_format` for custom colormap formats, replace `with_label_format(None)` with `without_value_labels()`, and use an axis formatter closure for localized or arbitrary UTF-8 labels. Reflect stores typed `I32NumericSettings`, `U32NumericSettings`, `F32NumericSettings`, or `F64NumericSettings`; width/precision presets are fallible `try_*` methods. |
...
| Core `tracing`, `dear_imgui_rs::logging`, or exported logging macros | Configure the subscriber in the application; enable `dear-imgui-wgpu/tracing` only when backend events are wanted. Thanks [@DBLouis](https://github.com/DBLouis) for [PR #42](https://github.com/Latias94/dear-imgui-rs/pull/42). |
| Implicit source/prebuilt/WASM combinations | Select the complete route on the safe crate. `prebuilt` and Test Engine are native-only, Test Engine builds from source, supported browser extensions forward `wasm`, and blueprints require native `stack-layout`. |
| WASM provider artifacts or import maps using `imgui-sys-v0` | Rebuild the provider and use `imgui-sys-v1`. Renaming or remapping a v0 artifact is insufficient because v1 identifies the checked native numeric formatting and parsing contract. |
...
| Raw aggregate `PlatformIO` callbacks or a pre-0.16 native prebuilt | Rust callback signatures use pointer/out parameters while C++ owns the by-value ABI; native archives must advertise `platform-io-aggregate-hooks-v3`, and older archives are rejected before linking. |
...
| `TestRunner::run_with_renderer` or manual Test Engine `pre_swap` / `post_swap` calls | Implement `TestFrameDriver` and use `run_graphical`, which owns `render -> pre-swap -> present -> post-swap`; integrations that own the test queue use `TestEngine::drive_frame`. Headless runs are explicitly virtual. Screenshot/video support is opt-in through the `capture` feature and `CapturingTestFrameDriver`. |
...
- Added a checked unknown-count list-clipper protocol, bounded Test Engine runners with explicit graphical and virtual presentation paths, run-scoped framebuffer capture providers, typed lifecycle/results, label-based table resizing, docking automation, and terminal/error logging. Opt-in runtime gates now run the exact upstream native-default, docking, and viewport suite manifests and require every registered test to succeed.
...
- Releases now run the fixed same-commit gate, candidate-bound resumable 27-crate crates.io Trusted Publishing, exact prebuilt asset staging with checksums, and reject stale existing GitHub Release assets before upload through one protected workflow.
- ImNodes editor frames now separate pre-submission configuration from node submission, retain explicit editor lifetimes, validate node/pin/link identities and minimap inputs before FFI, and close forgotten nested tokens safely instead of delegating invalid scope states to native assertions.
...
- Bevy additional Contexts now use namespaced `ImguiContextPass` schedules, preventing accidental nested execution of `Update` or another application schedule. `ImguiAppExt::shutdown_imgui` deterministically drains Context, renderer, and viewport ownership without running user schedules; ordinary drop synchronously releases already-detachable Contexts.
...
- SDL3 event processing now owns text payloads across the native call and prevents safe Rust from submitting fabricated pointer-bearing union variants to the official backend.
- SDL3 platform ownership now excludes concurrent runtimes before they can compete for process-wide cursor, capture, IME, and hint state. Viewport teardown no longer retains or later dereferences destroyed native viewport addresses, and entry guards preserve callback-replacement faults across errors and panics.
...
- WGPU secondary viewports now honor the renderer's surface, MSAA, and depth-stencil configuration and suspend at zero size instead of forcing `Fifo` or reusing incompatible attachments. A lost surface is reported as a terminal runtime fault so applications can distinguish surface-only recovery from full Device recovery through their own WGPU callback. Fixes [#51](https://github.com/Latias94/dear-imgui-rs/issues/51), reported by [@DrBarnabus](https://github.com/DrBarnabus).
- WGPU now derives frame-resource ownership from the Context render epoch and allocates distinct vertex, index, uniform, and sampler bindings for every main or secondary viewport draw. Repeated preparation is idempotent, stale callbacks cannot reuse the preceding frame, viewports cannot overwrite one another's queued GPU inputs, and pending surface frames are destroyed before their parent surface during terminal cleanup.
- `dear-app` now recovers `Lost`/`Outdated` main surfaces before UI work, skips `Timeout`/occluded frames without advancing application or Test Engine state, and reports validation, internal, and out-of-memory GPU faults instead of presenting a partial frame.
- WGPU and SDL3/Glow multi-viewport examples now finish and validate secondary platform windows before acquiring or presenting the main surface; Test Engine timing brackets only the main presentation.
- Winit and Bevy native multi-viewport docking now keep client coordinates, DPI-time event ordering, Context-wide focus, capture handoff, decorated-window placement, camera clear policy, and native move/resize feedback synchronized across mixed-DPI desktops. Bevy separates geometry requests from authoritative native observations at each frame boundary, keeping dock previews aligned when a window manager clamps, delays, or externally moves a window. Wayland explicitly falls back to docking inside the host window because it cannot provide global desktop positions. Addresses [#60](https://github.com/Latias94/dear-imgui-rs/issues/60), reported by [@lysenika](https://github.com/lysenika).
...
- Bevy now transitions every ImGui-routed window before startup presentation, preventing Vulkan validation failures while render pipelines and custom font textures warm up.
- `Ui::push_font` now uses the font's configured reference size, while `push_font_with_size(..., 0.0)` remains the explicit way to preserve the current size. Fixes [#58](https://github.com/Latias94/dear-imgui-rs/issues/58), reported by [@lysenika](https://github.com/lysenika).
...
- Refreshed the cimgui extension pins for the Dear ImGui 1.92.9 metadata model and updated ImGuizmo with its 1.92 DrawList, multi-viewport, multi-gizmo, hit-testing, and camera-ray fixes. `compute_mouse_ray` exposes the new context-free picking helper with checked inputs and results.
- Fixed Test Engine capture startup under undefined-behavior sanitizers, reclaimed quarantined callback panic payloads without unwinding across C ABI boundaries, and ensured ImGuIZMO.quat is compiled exactly once in native and Emscripten provider builds.
- ImPlot annotation and axis-tag text now treats `%` as literal input, and ImNodes WASM exposes string persistence instead of disk methods that cannot succeed in browser builds.
- Numeric Drag, Slider, InputScalar, ImPlot, and reflect format paths now reject type mismatches, incompatible flags, extra conversions, dynamic arguments, `%n`, interior NULs, excessive width or precision, and oversized formats before FFI. Core formatting keeps UTF-8 decorations outside locale-sensitive `printf` calls; ImPlot formats are ASCII-only because upstream forwards the complete string. Native and WASM providers share the same checked numeric parsing overlay, and core prebuilt manifests include its source-contract hash so older archives cannot bypass the fix.
...
- Docking APIs now reject disabled docking and duplicate same-frame dockspace submissions before entering native code. `DOCKING_ENABLE` is fixed after the first frame so runtime toggles cannot silently destroy live layouts.
